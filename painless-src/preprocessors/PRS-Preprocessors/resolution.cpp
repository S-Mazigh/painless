...
bool preprocess::preprocess_resolution() {    
    ...
    for (int i = 1; i <= clauses; i++) {
        ...
        clause_delete[i] = 0;
        for (int j = 0; j < l; j++) {
            if (clause[i][j] > 0) occurp[abs(clause[i][j])].push_back(i);
            else occurn[abs(clause[i][j])].push_back(i);
        }
    }
    for (int i = 1; i <= vars; i++)
            ...
            for (int j = 0; j < op; j++) {
                ...
                for (int k = 0; k < l; k++) {
                    ...
                    if (!clean[v] && seen[v] != seen_flag)
                        vars.push_back(v), seen[v] = seen_flag;
                }
            }
            for (int j = 0; j < on; j++) {
    ...
    for (int i = 1; i <= clauses; i++) {
        ...
        int l = clause[i].size();
        for (int j = 0; j < l; j++) {
            res_clause[res_clauses].push_back(pnsign(clause[i][j]) * mapfrom[abs(clause[i][j])]);
        }
    }
    resolutions = r;
