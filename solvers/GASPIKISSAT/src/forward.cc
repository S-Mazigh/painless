...
forward_subsume_all_clauses(kissat *solver)
{
	...
	struct flags *saved = (struct flags *)kissat_calloc(solver, solver->vars, sizeof *saved);
...
void kissat_forward_subsume_during_elimination(kissat *solver)
{
	START(forward);
...
bool kissat_forward_subsume_temporary(kissat *solver)
{
