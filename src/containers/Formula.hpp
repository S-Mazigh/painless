...
public:
   ...
   * @param index The index of the clause to be removed from the occurrence
   * list.
   * @return True if the operation was successful.
   */
  bool delete_nonUnit_occurrence(int lit, unsigned int index);
  ...
  {
    return skipzero_span<unsigned int>(this->occurrenceLists.at(LIT_IDX(lit)));
  }
  ...
  {
    return this->nonUnits.getRowsCount() - this->deletedClausesCount;
  }
  ...
  {
    return this->getUnitCount() + this->getNonUnitsCount();
  }
...
private:
  ...
  std::vector<std::vector<unsigned int>>
    occurrenceLists;         ///< Occurrence lists for each literal.
  unsigned int varCount = 0; ///< Number of variables in the formula.
  unsigned int deletedClausesCount =
    0; ///< Count of deleted clauses for efficient non-unit clause counting
};
