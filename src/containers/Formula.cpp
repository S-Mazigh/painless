...
Formula::push_clause(const std::vector<int>& clause)
{
  if (clause.size() == 1) {
    ...
    for (int lit : clause) {
      ...
      if (index >= occurrenceLists.size())
        occurrenceLists.resize(index + 1);
      /* index of last pushed clause is size - 1 */
      occurrenceLists.at(index).push_back(nonUnits.getRowsCount() - 1);
    }
  }
  return true;
}
...
Formula::delete_lit_nonUnit(unsigned int index, int dlit)
{
  ...
  LOGDVECTOR2(clause.data(),
              clause.size(),
              "Literal %d is in index %u in clause %u",
              dlit,
              lit_index,
              index);
  if (!nonUnits.delete_element(index, lit_index)) {
    LOGD1("The formula is UNSAT at unit propagation, clause %u is empty",
          index);
    return false;
  }
  ...
  if (this->getNonUnitEfficientSize(index) == 1) {
    ...
    LOGD1("New unit clause to be added: %d", newunit);
    if (!this->insert_unit(newunit))
      return false;
  }

  // Remove clause from occurrences of lit
  return delete_nonUnit_occurrence(dlit, index);
}

bool
Formula::delete_nonUnit_occurrence(int dlit, unsigned int index)
{
  // Remove clause from lit occurrence lit by making it zero
  auto& tempRef = this->occurrenceLists.at(LIT_IDX(dlit));
  std::find(tempRef.begin(), tempRef.end(), index).operator*() =
    0; /* need to find a better contiguous data structure for quick search*/

  /* Detect pure literals if occurrenceLists is empty */
  if (std::all_of(tempRef.begin(), tempRef.end(), [](unsigned int index) {
        return index == 0;
      })) {
    LOGD1("Detected Pure Literal: %d", -dlit);
    /* Check for non occuring variables (both lits are pure) */
  }

  LOGDVECTOR2((int*)tempRef.data(),
              tempRef.size(),
              "Removed %d for literal %d: ",
              index,
              dlit);

  // if new real size == 0 => -lit is a pure literal, need to iterate to check
  // if all zeros !! Add a deleted count in back or front (abstract to ignore it
  // needed) or a separate sizes as in twoDvector
  return true;
}
...
Formula::delete_nonUnit(unsigned int index)
{
  ...
  for (auto lit : clauseToDelete) {
    delete_nonUnit_occurrence(lit, index);
  }
...
Formula::shrink_structures()
{
  ...
  unsigned int initialNonUnitsSize =
    nonUnits.end(nonUnits.getRowsCount() - 1) - nonUnits.begin(0);
  unsigned int initialOccurenceListsSize = this->occurrenceLists.size();
  ...
  for (size_t i = 0; i < occurrenceLists.size(); ++i) {
    auto& occurrenceList = occurrenceLists[i];
    // Remove all zero elements
    occurrenceList.erase(
      std::remove(occurrenceList.begin(), occurrenceList.end(), 0),
      occurrenceList.end());
    // Shrink the vector to release unused memory
    occurrenceList.shrink_to_fit();

    // Detect pure literals (TODO, update units, and manage deleted vars)
    if (occurrenceList.empty()) {
      LOGD1("PureLiteral: %d", IDX_LIT(i));
      ++pureLiteralsCount;
    }
  }
  ...
  unsigned int finalNonUnitsSize =
    nonUnits.end(nonUnits.getRowsCount() - 1) - nonUnits.begin(0);
  unsigned int finalOccurenceListsSize = this->occurrenceLists.size();
  ...
  LOGSTAT("Formula Shrink Results (before -> after): nonUnits: %u -> %u "
          "(clauses: %u->%u), occurrenceLists: %u -> %u "
