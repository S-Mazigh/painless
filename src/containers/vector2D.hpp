...
public:
  // Custom iterator that skips zero elements, add a m_ptr attribute for midway
  // starting points ?
  class Iterator
  {
  public:
...
public:
   ...
   * @return False if the entire row is deleted after this operation, true
   * otherwise.
   */
  bool delete_element(row_id_t rowId, row_size_t offset);

  /**
   * @brief Cleans up the 2D vector by removing deleted rows and compacting
   * data.
   */
  void cleanup();
...
private:
  ...
  std::vector<data_size_t>
    indexes; ///< Starting indexes of rows in the data store.
  std::unordered_map<row_size_t, data_size_t>
    freeRows; ///< Map of free rows for potential reuse.
};
...
//     // A map size to index is to be generated by iterating over all the
//     clauses std::unordered_map<row_size_t,data_size_t> rowBySize;
//     // Keep indexes of deleted,
//     std::unordered_map<data_size_t,row_size_t> deletedRows;
//     // In the loop: Check if the row need to be shifted because of zeroes
//     before copying it row_id_t rowCount = this->indexes.size(); for(row_id_t
//     rowId = 0; rowId < rowCount; rowId++)
//     {
//         if(!ROW_SIZE(rowId))
//         {
//             // Delete row
...
//         - Find a row or multiple rows with index > drow.fisrt that could fit
//         in drow.second, using rowBySize map
//         - If one of the filling rows is at index cutId - 1, update cutId to
//         the index of the row (if multiple rows, check if they are not
//         consecutive)
//         - Update the indexes and capacities vector accordingly. The
//         capacities become their sizes, since we are looking for perfect fits
