...
kissat_init(void)
{
  ...
  //--------------------------
#endif
  // printf("[In Kissat_Mab] After kissat_init with solver=%p solver->statistics.searches = %d\n", solver, solver->statistics.searches);
  return solver;
}
...
void kissat_release(kissat *solver)
{
  ...
  if (leaked)
    if (!getenv("LEAK"))
    {
      printf("internally leaking %" PRIu64 " bytes\n", leaked);
...
void kissat_init_shuffle(kissat *solver, int max_var)
{
...
void kissat_reserve(kissat *solver, int max_var)
{
  ...
  // kissat_init_shuffle(solver, max_var); // better to test initial phase of kissat
}
