...
struct kissat
{
  ...
  int (*cbkImportClause)(void *, unsigned *, cvec *);
  void (*cbkExportClause)(void *, unsigned, cvec *); // callback for clause learning

  int *init_phase;
  int max_var; // from cnf file
#ifdef LOGGING
  bool compacting;
#endif
  bool extended;
  ...
  */
   ...
   * Meaning: if everything is initialized to zero, in kissat_add, it marks an already seen unassigned literal by 1, and its negation by -1
   * It is useful to detect tautologies, and it is reset to zero after the end of an original clause
   */
  mark *marks;
  ...
  */
  ...
  struct temporary
  * A clause is first initialize here before calling kissat_new_original_clause or kissat_new_redundant_clause, which will call new_clause using the data saved here
  {
    bool satisfied;
    bool shrink;
    bool trivial;
